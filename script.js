...
newBookName.addEventListener("input", () => {
  const validName = newBookName.value.length > 3;
  if (validName) {
    newBookName.classList.add("valid");
  }
});

newAuthor.addEventListener("input", () => {
  const validAuth = newAuthor.value.length > 3;
  if (validAuth) {
    newAuthor.classList.add("valid");
  }
});

newGenre.addEventListener("input", () => {
  const validAuth = newGenre.value.length > 3;
  if (validAuth) {
    newGenre.classList.add("valid");
  }
});
...
function allValid() {
  let valids = 0;
  for (let i = 0; i < 3; i++) {
    if (arguments[i].classList.value === "valid") {
      valids++;
    }
  }
  if (valids === 3) {
...
let theHobbit = new book(
  "53feeb9f-b87c-43b4-b54d-51441e01f4f3",
  "J.R.R. Tolkien",
  "The Hobbit",
  "Fantasy",
  true
...
let bloodMeridian = new book(
  "cc2e6e4c-6120-45b2-bb39-e44c6fdaf162",
  "Cormac McCarthy",
...
// New book adding function

function feedTheConstructor() {
...
addBookBtn.addEventListener("click", (event) => {
  const isValid = allValid(newBookName, newAuthor, newGenre);
  if (isValid === true) {
    feedTheConstructor();
  } else {
  }
